...
{?>
  <tr>
	...
	<td><?php
	 if($row->tampilkan!=1){echo"[ Tidak di blok ] ><a href=\"index.php?p=user&amp;mode=on&amp;id=$row->id\">Blok</a>";}
if($row->tampilkan!=0){echo"[ Di blok ] ><a href=\"index.php?p=user&amp;mode=off&amp;id=$row->id\">Jangan di blok</a>";} ?></td>
...
<?php }
	$total_results = count(DB::con()->query("SELECT COUNT(*) as Num FROM user");
